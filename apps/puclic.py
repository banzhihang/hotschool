...
class Authtication(BasicAuthentication):
    ...
    def authenticate(self, request):
        ...
        # token不存在
        if not token:
            raise exceptions.AuthenticationFailed({'msg':'用户未登录','code':1})
        try:
            # 解析token,若出现异常，则说明token被篡改过，属于非法
            payload = jwt_decode_handler(token)
        except Exception:
            raise exceptions.AuthenticationFailed({'msg':'用户账号异常','code':2})
        # 获得用户id
        user_id = jwt_get_user_id_from_payload_handler(payload)
        try:
            user = User.objects.get(pk=user_id)
        except User.DoesNotExist:
            raise exceptions.AuthenticationFailed({'msg':'用户不存在','code':3})
        else:
            return user, None

    def authenticate_header(self, request):
        """重载该方法,去除response头部中的www_authenticate_realm,避免未认证弹窗"""
        return None
...
def check_undefined(func):
    """检查get请求中是否有undefined的装饰器,有就直接返回错误"""
